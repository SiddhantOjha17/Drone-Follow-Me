...
class Logger:
    ...
    def __init__(self, path="../vids/"):
        ...
        self.videoWriter = cv2.VideoWriter(filename=filename,  # Provide a file to write the video to
            fourcc=cv2.VideoWriter_fourcc('P', 'I', 'M', '1'),  # bring up codec dialog box
            fps=30,
            frameSize=(640, 480))
    ...
    def log(self, frame, vehicle):
        ...
        if vehicle:
            self.logFile.write(str(self.frame_number) + "," + str(datetime.datetime.today()) + "," + get_attitude_string(vehicle) + "," + ";\n")
        else:
