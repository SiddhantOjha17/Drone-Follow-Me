...
def discard_saturated_pixels(hsv):
    mask = cv2.inRange(hsv, np.array((0., 150., 0.)),
...
def detect_biggest_polygon(binary_img):
    # contour detection will mess up the image
    binary_img_to_process = binary_img.copy()
    contours, _ = cv2.findContours(
        binary_img_to_process, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
...
def target_coordinates(target):
    print("target coordinate", type(target))
    if target == None:
        return None
    contour_centroid = cv2.moments(target)
    print("centroid", contour_centroid)
    print("type of centroid", type(contour_centroid))
    try:
        cx, cy = int(contour_centroid['m10'] / contour_centroid['m00']
                     ), int(contour_centroid['m01'] / contour_centroid['m00'])
        return (cx, cy)
